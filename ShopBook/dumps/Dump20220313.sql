...
INSERT INTO `permissions` VALUES ('EG0000001G0001','Furniture Collage'),('EG0000001G0002','Women Clothes'),('EG0000001G0003','Fashion Men');
...
/*!50003 SET sql_mode              = 'PIPES_AS_CONCAT,ANSI_QUOTES,IGNORE_SPACE,ORACLE,NO_KEY_OPTIONS,NO_TABLE_OPTIONS,NO_FIELD_OPTIONS' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER="shopbook"@"%"*/ /*!50003 TRIGGER `shopbook`.`roles_BEFORE_INSERT` BEFORE INSERT ON `roles` FOR EACH ROW
BEGIN
    declare xx varchar(5) default null;
    if  new.TYPE = 'C' then
select MIN(c.start) FROM (
SELECT substr(a.ID,2)+1 AS start
    FROM roles AS a, roles AS b
    WHERE substr(a.ID,2) < substr(b.ID,2)
    and substr(a.ID,1,1) = 'C' 
    GROUP BY substr(a.ID,2)
    HAVING start < MIN(substr(b.ID,2)) ) c into xx;
  if xx is null then
select COALESCE(max(substr(roles.ID,2)),0) +1 into xx
FROM roles where substr(roles.ID,1,1) = 'C';
end IF;
select case 
  WHEN  LENGTH(xx) = 1 
      then concat('000',xx)
...
delete from permissions where ID = old.shop_cat_code;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'PIPES_AS_CONCAT,ANSI_QUOTES,IGNORE_SPACE,ORACLE,NO_KEY_OPTIONS,NO_TABLE_OPTIONS,NO_FIELD_OPTIONS' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER="shopbook"@"%"*/ /*!50003 TRIGGER `shopbook`.`shop_category_admins_AFTER_INSERT` AFTER INSERT ON `shop_category_admins` FOR EACH ROW
BEGIN
declare v_role varchar(14) default null;
insert into roles_permissions (ROLE, ID) values (
(select ID from permissions where ID = substr(new.SHOP_CAT_SUB_CODE,1,14)),
(select ROLE_ID from users_roles where USER_ID = new.USER_ID));
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'PIPES_AS_CONCAT,ANSI_QUOTES,IGNORE_SPACE,ORACLE,NO_KEY_OPTIONS,NO_TABLE_OPTIONS,NO_FIELD_OPTIONS' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER="shopbook"@"%"*/ /*!50003 TRIGGER `shopbook`.`shop_category_admins_BEFORE_DELETE` BEFORE DELETE ON `shop_category_admins` FOR EACH ROW
BEGIN
delete from roles_permissions 
where ROLE = substr(old.SHOP_CAT_SUB_CODE,1,14)
and ID = (select ROLE_ID from users_roles where USER_ID = old.USER_ID);
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
INSERT INTO `sys_parameters` VALUES (1,NULL,NULL,NULL,'localhost','mail.mymr.health',465,NULL,'registeration@mymr.health','registeration',NULL,'/home/mfayed/Documents/shopbook/ShopBook/ShopdemoData/','NoImage.png','MyMedicalRecord123!','en','http://127.0.0.1:7101/shopbookHome/shop/adf.task-flow?adf.tfDoc=%2FWEB-INF%2Freset-pwd-tf.xml&adf.tfId=reset-pwd-tf&pOneTimeCode=');
...
/*!50003 SET sql_mode              = 'PIPES_AS_CONCAT,ANSI_QUOTES,IGNORE_SPACE,ORACLE,NO_KEY_OPTIONS,NO_TABLE_OPTIONS,NO_FIELD_OPTIONS' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER="shopbook"@"%"*/ /*!50003 TRIGGER `shopbook`.`users_AFTER_UPDATE` AFTER UPDATE ON `users` FOR EACH ROW
BEGIN
declare _exist int(2) default 0;
if new.IS_ADMIN = 'C'  and new.ONE_TIME_CODE is null then
select count(USER_ID) into _exist
FROM users_roles where USER_ID = new.USER_ID;
	if _exist = 0 then
	insert into roles(TYPE,ROLE) 
    values (new.IS_ADMIN,'CatAdmin');
    
    insert into user_roles(USER_ID,ROLE_ID)
    select old.USER_ID,'C0000';
	insert into users_roles (USER_ID,ROLE_ID)
	select old.USER_ID, max(ID) from roles
	where TYPE = 'C';
	end if;
elseif new.IS_ADMIN in ('U','O') and new.ONE_TIME_CODE is null then
select count(USER_ID) into _exist
FROM users_roles where USER_ID = new.USER_ID;
	if _exist = 0 then
	insert into users_roles (USER_ID,ROLE_ID)
	values (new.USER_ID,concat(new.IS_ADMIN,'0000'));
    end if;
    elseif new.IS_ADMIN in ('S') and new.ONE_TIME_CODE is null then
select count(USER_ID) into _exist
FROM users_roles where USER_ID = new.USER_ID;
	if _exist = 0 then
	insert into users_roles (USER_ID,ROLE_ID)
	values (new.USER_ID,'S0000');
    insert into users_roles (USER_ID,ROLE_ID)
	values (new.USER_ID,'C0000');
    end if;
...
-- Dump completed on 2022-03-13 16:51:41
